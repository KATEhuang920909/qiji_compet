...
import paddle
from data import load_dataset, load_dict_single, parse_decodes
...
if __name__ == "__main__":
    ...
    train_ds, dev_ds = load_dataset(
        datafiles=(
            os.path.join(args.data_dir, "train.0"),
            os.path.join(args.data_dir, "dev.0"),
        )
    )

    label_vocab = load_dict_single(os.path.join(args.data_dir, "tag.txt"))
    print(label_vocab)
    ...
    ernie = ErnieModel.from_pretrained(r"ernie-3.0-medium-zh")

    model = ErnieCrfForTokenClassification(ernie, 768, 256, len(label_vocab))
    ...
    preds = predict(model, dev_loader, dev_ds, label_vocab)
