...
with table1:
    ...
    with tab1:
        with st.form(key="1"):
            ...
            *规范个人言语行为，保护个人安全信息*
            ...
            contents = st.text_input('请输入文本', key=0)
        ...
        with st.form(key="2"):
            '''
                    ...
                    *规范个人言语行为，保护个人安全信息*
                    ```
            '''
            uploaded_file = st.file_uploader("请选择文本", type=["xlsx", "xls", "txt", "pdf"], key=1)
            submitted = st.form_submit_button('提交')
            if submitted:
    ...
    with tab3:
        with st.form(key="3"):
            ...
            '''
            # 上传图片并展示
            uploaded_file = st.file_uploader("请上传图片", type=["jpg", "png"], key=2)
            submitted = st.form_submit_button('提交')
            if submitted:
                if uploaded_file is not None:
                    # 将传入的文件转为Opencv格式
...
with table2:
        ...
        with st.form(key="4"):
            '''
                    ...
                    *鉴别网络不良信息，提高网络防范意识*
                    ```
            '''
            contents = st.text_input('请输入文本', key=3)
            submitted = st.form_submit_button('提交')
            if submitted:
                if contents:
                    st.write("数据预览：")
        ...
        with st.form(key="5"):
            '''
                    ...
                    *鉴别网络不良信息，提高网络防范意识*
                    ```
            '''
            uploaded_file = st.file_uploader("请选择文件", type=["xlsx", "xls", "txt", "pdf"], key=4)
            submitted = st.form_submit_button('提交')
            if submitted:
                if uploaded_file is not None:
                    name = uploaded_file.name.split(".")[-1]
    ...
    with tab3:
        with st.form(key="6"):
            ...
            '''
            # 上传图片并展示
            uploaded_file = st.file_uploader("请上传图片", type=["jpg", "png"], key=5)
            submitted = st.form_submit_button('提交')
            if submitted:
                if uploaded_file is not None:
                    # 将传入的文件转为Opencv格式
